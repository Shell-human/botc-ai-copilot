/* ==========================================================================
   promptBuilder.js - System/User Prompt 构建器 (Prompt Builder) v3.0
   职责：根据 gameState 构建完整的 AI Prompt 字符串
   v3.0: 采用数据字典驱动的国际化重构，合并中英文模板，消除数百行代码重复
   ========================================================================== */
...
const PROMPT_DICTIONARY = {
    zh: {
        baseDistStr: (d) => `场上标准配置：${d.townsfolk} 镇民, ${d.outsider} 外来者, ${d.minion} 爪牙, ${d.demon} 恶魔`,
        formatPlayerStatus: (p) => {
        ...
        formatGameLog: (log, idx) => `[第 ${idx + 1} 条事件] ${getLocalizedLog(log, "zh")}`,
        chatHistoryHeader: "\n=== 对话历史上下文 ===\n",
        chatHistoryItem: (idx, roleStr, truncatedContent) => `[第${idx+1}条] ${roleStr}:\n${truncatedContent}`,
        chatRoleUser: "用户",
        chatRoleAssistant: "AI 助手",
        analysisHistoryHeader: "\n=== 历史 AI 战术副驾驶推演记忆 ===\n",
        analysisHistoryFooter: "\n\n（请务必参考上述历史输出，保持战术建议与前序推演的一致性与连贯性。除非出现硬性的新证据或局势剧变，否则请不要否定自己之前的结论。）\n",
        analysisHistoryItem: (idx, typeStr, input, truncatedOutput) => `[历史记忆 ${idx+1}] Interaction Type: ${typeStr}\n- 用户输入: "${input}"\n- 你当时的回复输出:\n${truncatedOutput}\n---`,
        analysisTypeChat: "自由提问对话",
        analysisTypeRound: "局势进展推演",
        
        systemRolePromptChat: (seat, role, isEvil) => `你是一名极其资深的《血染钟楼》逻辑学家、顶尖博弈大宗师。你正在作为一名玩家（即玩家 ${seat} 号，实际身份为【${isEvil ? '邪恶阵营' : '善良阵营'}】的 ${role}）的"AI战术副驾驶"帮助他解答规则问题，或者提供实时的个人角色打法与策略选择。
【对话模式】重要限制：
...
请直接、自然、亲切地用第一人称对话形式来回答他的问题，**绝对不要**在回复中包含 \`=== ANALYSIS ===\`、\`=== TIPS ===\` 这两个分类标签！直接在 Markdown 中给出一份详实、深入且充满战术智慧的直接回答即可！`,
        setupPromptChat: (role, isEvil, distStr, consoleText) => `- 我的真实身份：${role}（${isEvil ? '【邪恶阵营】' : '善良阵营'}）\n- 游戏人数角色标准分布：${distStr}\n- 当前处于：与 AI 自由提问对话模式\n- 用户对话问题：${consoleText}`,
        
        systemRolePromptEvil: (seat, role, bluffsStr, teammatesStr) => `你是一名极其资深的《血染钟楼》逻辑学家、顶尖博弈高手。你正在作为一名玩家（即玩家 ${seat} 号，实际身份为【邪恶阵营】的 ${role}）的"AI战术副驾驶"帮助他和邪恶同伴打配合、欺骗好人阵营、混淆视听并获得最终胜利。
...
- You必须假设在场的好人玩家依然把他们当作普通好人或身份未知的玩家来对待。
...
- 分析哪些好人的宣称是真实的，哪些是我们可以利用逻辑漏洞击破的！`,
        setupPromptEvil: (role, distStr, bluffsStr, teammatesStr) => `- 我的真实身份：${role}（【邪恶阵营】）\n- 游戏人数角色标准分布：${distStr}\n- 说书人给的 3 个好人伪装 (Bluffs)：${bluffsStr}\n- 已确认的邪恶队友：${teammatesStr}`,
        
        systemRolePromptGood: (seat, role, distStr) => `你是一名极其资深的《血染钟楼》逻辑学家、顶尖博弈高手。你正在作为一名玩家（即玩家 ${seat} 号，真实身份为【善良阵营】的 ${role}）的"AI战术副驾驶"帮助他梳理局势、检测逻辑冲突并找出隐藏的恶魔与爪牙。
请站在善良阵营的绝对立场，帮他寻找逻辑漏洞，防备邪恶阵营的欺骗！你可以结合该游戏人数的【标准配置人数分布（${distStr}）】和场上起跳外来者、爪牙的数量，来进行排除法推演。`,
        setupPromptGood: (role, distStr) => `- 我的真实身份：${role}（善良阵营）\n- 游戏人数角色标准分布：${distStr}`,
        
        scriptHeader: (name) => `目前我们正在进行的游戏剧本是：《${name}》。`,
        coreLawsHeader: "\n=== 血染钟楼核心推理钢性法则 ===\n",
        characterListHeader: "该剧本包含的角色及其具体能力定义（注意：在不同的剧本中，相同角色的具体能力和配置可能有微调差异，请以此为准）：",
        configLabel: "当前游戏基础配置",
        statusLabel: "当前场上所有玩家状态信息",
        logsLabel: "截至目前发生的事件日志记录",
        
        footerChat: (consoleText) => `【用户直接对话提问】："${consoleText}"
请针对我的提问给予直接、深度、充满战术博弈智慧的详细解答。请直接用精美的 Markdown 格式输出，可以使用列表、粗体、分割线和图标。绝对不要输出任何形如 === ANALYSIS === 的分隔标签！`,
        footerAnalysis: (consoleText, myRole, isEvil, demonName, scriptName) => `请帮我根据最新的变化"${consoleText}"，输出三个模块的分析。
...
这里提供【行动建议与防猝死提示】。
分析要点：
1. 战术行动：我（真实角色为【${myRole}】，当前阵营为【${isEvil ? '邪恶阵营' : '善良阵营'}】）在白天最应该去私聊场上的哪些玩家以达到核对或掩盖身份的目的？在投票环节中，我们今天最应该采取何种投票策略，或是应该诱导大家将票挂在谁头上？
2. 针对我们正在玩的《${scriptName}》剧本角色（如 ${demonName} 恶魔，以及爪牙与村民的博弈关系），指出本局本轮次对我们而言最紧迫的防守或进攻漏洞。
...
- 仅输出 JSON 代码块，不要在此之后附加任何其他内容。`
    ...
    en: {
        baseDistStr: (d) => `Standard configuration: ${d.townsfolk} Townsfolk, ${d.outsider} Outsiders, ${d.minion} Minions, 1 Demon`,
        formatPlayerStatus: (p) => {
            const roleNameEn = p.claim === '未知' ? 'Unknown' : (ROLE_TRANSLATIONS[p.claim] || p.claim);
            const alignStr = p.alignment === 'good' ? 'Good' : (p.alignment === 'evil' ? 'Evil' : 'Unknown');
            return `- [Seat ${p.seat}] Name: ${p.name}, Status: ${p.alive ? 'Alive' : '❌ Dead'}, Claimed Character: ${roleNameEn}, Suspected Alignment: ${alignStr}, Drunk/Poisoned: ${p.poisoned ? 'Yes' : 'No'}, Notes: ${p.note || 'None'}`;
        },
        formatGameLog: (log, idx) => `[Event ${idx + 1}] ${getLocalizedLog(log, "en")}`,
        chatHistoryHeader: "\n=== CHAT CONVERSATION HISTORY ===\n",
        chatHistoryItem: (idx, roleStr, truncatedContent) => `[#${idx+1}] ${roleStr}:\n${truncatedContent}`,
        chatRoleUser: "User",
        ...
        analysisHistoryItem: (idx, typeStr, input, truncatedOutput) => `[Memory ${idx+1}] Interaction Type: ${typeStr}\n- User Input: "${input}"\n- Your Previous Response:\n${truncatedOutput}\n---`,
        analysisTypeChat: "Direct Q&A Chat",
        analysisTypeRound: "Round Seating Deduction",
        
        systemRolePromptChat: (seat, role, isEvil) => `You are an extremely experienced Blood on the Clocktower logician and strategic mastermind. You are acting as the "AI Tactical Copilot" for Player ${seat} (real role: ${role}, alignment: ${isEvil ? 'Evil' : 'Good'}).
【Chat Mode】IMPORTANT:
...
Please reply directly and conversationally in a professional strategy advisor style. **DO NOT** include the '=== ANALYSIS ===' or '=== TIPS ===' tags in your response. Just write a comprehensive, highly strategic reply directly in Markdown.`,
        setupPromptChat: (role, isEvil, distStr, consoleText) => `- My Real Role: ${role} (${isEvil ? 'Evil Team' : 'Good Team'})\n- Standard Character Distribution: ${distStr}\n- Active Mode: Direct Chat / Q&A\n- User Query: ${consoleText}`,
        
        systemRolePromptEvil: (seat, role, bluffsStr, teammatesStr) => `You are an extremely experienced Blood on the Clocktower logician and strategic mastermind. You are acting as the "AI Tactical Copilot" for Player ${seat} (real role: ${role}, alignment: Evil).
...
- Deduce which claims from Good players are real, and highlight logical contradictions to exploit.`,
        setupPromptEvil: (role, distStr, bluffsStr, teammatesStr) => `- My Real Role: ${role} (Evil Team)\n- Standard Character Distribution: ${distStr}\n- 3 Demon Bluffs from Storyteller: ${bluffsStr}\n- Confirmed Evil Teammates: ${teammatesStr}`,
        
        systemRolePromptGood: (seat, role, distStr) => `You are an extremely experienced Blood on the Clocktower logician and strategic mastermind. You are acting as the "AI Tactical Copilot" for Player ${seat} (real role: ${role}, alignment: Good).
...
You should leverage the standard player count distribution (${distStr}) combined with active claims to execute deduction by elimination.`,
        setupPromptGood: (role, distStr) => `- My Real Role: ${role} (Good Team)\n- Standard Character Distribution: ${distStr}`,
        
        scriptHeader: (name) => `Current Script: "${name}".`,
        coreLawsHeader: "\n=== BOTC CORE DEDUCTION LAWS ===\n",
        characterListHeader: "Character descriptions in this script (use these definitions for logical constraints):",
        configLabel: "Game Configuration",
        statusLabel: "Player Statuses",
        logsLabel: "Game Logs",
        
        footerChat: (consoleText) => `【USER DIRECT QUERY】: "${consoleText}"
Please answer my question directly and conversationally in Markdown. Feel free to use lists, bold text, and clear formatting. Do NOT output any tags like === ANALYSIS === or similar.`,
        footerAnalysis: (consoleText, myRole, isEvil, demonName, scriptName) => `Please provide depth analysis based on the latest update: "${consoleText}".
...
Guidelines:
1. Tactical Actions: I am a ${myRole} (${isEvil ? 'Evil Team' : 'Good Team'}). Who should I private chat today to verify or hide alignment? What voting strategies should we pursue today, or whom should we nominate/frame?
2. Address script-specific dynamics (e.g. how to handle the ${demonName} demon, Minion-Townsfolk power struggles).
...
- If the user says someone "poisoned" / "drunk", set "poisoned": true。
...
export function constructPrompt(consoleText, isChat = false) {
    ...
    const isEn = lang === "en";
    const dict = PROMPT_DICTIONARY[lang];
    
    const currentScript = isEn ? SCRIPTS_DATA_EN[gameState.scriptName] : SCRIPTS_DATA[gameState.scriptName];
    ...
    const distStr = dict.baseDistStr(dist);
    ...
    // 格式化玩家状态与日志
    const playerStatuses = gameState.players.map(p => dict.formatPlayerStatus(p)).join('\n');
    const gameLogs = gameState.logs.map((log, idx) => dict.formatGameLog(log, idx)).join('\n');
    ...
    if (isChat) {
        if (gameState.chatMessages && gameState.chatMessages.length > 0) {
            ...
            aiHistoryPrompt = dict.chatHistoryHeader +
                historyToInclude.map((msg, idx) => {
                    const roleStr = msg.role === 'user' ? dict.chatRoleUser : dict.chatRoleAssistant;
                    const truncatedContent = msg.content.length > MAX_HISTORY_OUTPUT_LENGTH
                        ...
                        : msg.content;
                    return dict.chatHistoryItem(idx, roleStr, truncatedContent);
                }).join('\n\n---\n\n') + '\n';
        }
    } else {
        if (gameState.aiOutputs && gameState.aiOutputs.length > 0) {
            const historyToInclude = gameState.aiOutputs.slice(-6);
            aiHistoryPrompt = dict.analysisHistoryHeader +
                historyToInclude.map((item, idx) => {
                    const typeStr = item.type === "chat" ? dict.analysisTypeChat : dict.analysisTypeRound;
                    const truncatedOutput = item.output.length > MAX_HISTORY_OUTPUT_LENGTH
                        ...
                        : item.output;
                    return dict.analysisHistoryItem(idx, typeStr, item.input, truncatedOutput);
                }).join('\n\n') + dict.analysisHistoryFooter;
        }
    }
    ...
    if (isChat) {
        systemRolePrompt = dict.systemRolePromptChat(gameState.mySeat, isEn ? myRoleEn : gameState.myRole, isEvil);
        setupPrompt = dict.setupPromptChat(isEn ? myRoleEn : gameState.myRole, isEvil, distStr, consoleText);
    } else if (isEvil) {
        const activeBluffs = (gameState.evilBluffs || []).filter(Boolean);
        const bluffsStr = isEn 
            ? (activeBluffs.length > 0 ? activeBluffs.map(b => ROLE_TRANSLATIONS[b] || b).join(', ') : "None declared")
            : (activeBluffs.length > 0 ? activeBluffs.join('、') : "未填报");
        
        const teammatesStr = teammates.map(t => {
            const tClaimStr = isEn ? (ROLE_TRANSLATIONS[t.claim] || t.claim) : t.claim;
            return isEn 
                ? `Seat ${t.seat} (${t.name || 'Unnamed'}, Claimed: ${tClaimStr})`
                : `${t.seat}号玩家 (${t.name || '未命名'}, 宣称: ${tClaimStr})`;
        }).join(isEn ? ', ' : '、') || (isEn ? "No teammates marked yet" : "暂未标记邪恶队友");

        systemRolePrompt = dict.systemRolePromptEvil(gameState.mySeat, isEn ? myRoleEn : gameState.myRole, bluffsStr, teammatesStr);
        setupPrompt = dict.setupPromptEvil(isEn ? myRoleEn : gameState.myRole, distStr, bluffsStr, teammatesStr);
    } else {
        systemRolePrompt = dict.systemRolePromptGood(gameState.mySeat, isEn ? myRoleEn : gameState.myRole, distStr);
        setupPrompt = dict.setupPromptGood(isEn ? myRoleEn : gameState.myRole, distStr);
    }

    // 核心法则块与角色描述列表
    const scriptHeader = dict.scriptHeader(currentScript.name);
    const coreLawsBlock = isChat ? "" : dict.coreLawsHeader + (isEn ? CORE_LOGIC_RULES_EN : CORE_LOGIC_RULES) + "\n";
    
    const charDescBlock = isChat ? "" : `${dict.characterListHeader}
Townsfolk:
${townsfolkDesc}
...
`;
    ...
    const footer = isChat
        ? dict.footerChat(consoleText)
        : dict.footerAnalysis(consoleText, isEn ? myRoleEn : gameState.myRole, isEvil, currentScript.demon[0], currentScript.name);
